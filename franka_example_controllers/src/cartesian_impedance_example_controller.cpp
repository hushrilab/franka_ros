...
// //   cartesian_stiffness_ = cartesian_stiffness_ * 0.8;
// //   cartesian_damping_ = cartesian_damping_ * 0.3;
...
//     std::cout <<"[START] Initial Angles"<< q_initial << std::endl;
  Eigen::Affine3d initial_transform(Eigen::Matrix4d::Map(initial_state.O_T_EE.data()));

  // set equilibrium point to current state
  position_d_ = initial_transform.translation();
//   std::cout<<"Initial Point"<<position_d_<<std::endl;
  orientation_d_ = Eigen::Quaterniond(initial_transform.linear());
//   std::cout<<"Initial Orient"<<orientation_d_.w()<<std::endl<<orientation_d_.x()<<std::endl<<orientation_d_.y()<<std::endl<<orientation_d_.z()<<std::endl;
...
// //
  ...
  error.head(3) << position - position_d_;  // first three elements are filled up
// << is symbol to fill up matrix: m << 1, 2, 3,
//                                      4, 5, 6,
//                                      7, 8, 9;

  // orientation error
  if (orientation_d_.coeffs().dot(orientation.coeffs()) < 0.0) {            // dot product to check if quaternions have same sign
    orientation.coeffs() << -orientation.coeffs();
  }
  // "difference" quaternion
  Eigen::Quaterniond error_quaternion(orientation.inverse() * orientation_d_);
  error.tail(3) << error_quaternion.x(), error_quaternion.y(), error_quaternion.z();     // last three entries of error vector are the qauterion difference
...
// My ideas
  ...
  derror << jacobian * dq; 
  derror.head(3) << derror.head(3) - velocity_d_;
//  
 /////////////////////////////////////////////////// compute control   /////////////////////////////////////////////////////////////
  ...
  pseudoInverse(jacobian.transpose(), jacobian_transpose_pinv);     // compute the inverse, but because not quadratic it's the pseudo_inversion

  // Cartesian PD control with damping ratio = 1
  tau_task << jacobian.transpose() *
                  (-cartesian_stiffness_ * error - cartesian_damping_ * derror);
...
// // //   // compute error to desired pose
// // //   // position error
// // //   Eigen::Matrix<double, 6, 1> r_e;
// // //   Eigen::Matrix<double, 6, 1> dr_e;
// // //   r_e.head(3) << position - position_d_;  
// // //   dr_e << jacobian * dq; 
// // //   dr_e.head(3) << dr_e.head(3) - velocity_d_;
// // //   
// // //   // orientation error
// // //   if (orientation_d_.coeffs().dot(orientation.coeffs()) < 0.0) {            // dot product to check if quaternions have same sign
// // //     orientation.coeffs() << -orientation.coeffs();
...
// // //   // "difference" quaternion
// // //   Eigen::Quaterniond error_quaternion(orientation.inverse() * orientation_d_);
// // //   r_e.tail(3) << error_quaternion.x(), error_quaternion.y(), error_quaternion.z();     // last three entries of error vector are the qauterion difference
// // //   // Transform to base frame
// // //   r_e.tail(3) << -transform.linear() * r_e.tail(3);
// // //   
// // //   // Desired torque
// // //   Eigen::VectorXd tau_d(7), tau_task(7);
// // //   tau_task << -jacobian.transpose() * (cartesian_stiffness_ * r_e + cartesian_damping_ * dr_e);
