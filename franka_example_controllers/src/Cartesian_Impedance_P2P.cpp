...
void CartesianImpedanceP2P::starting(const ros::Time& /*time*/) {
  ...
  orientation_init     =   initial_transform.rotation();
...
void CartesianImpedanceP2P::update(const ros::Time& time, const ros::Duration& period) {
  ...
  curr_position = transform.translation();
  curr_orientation = transform.rotation();
...
//position_d_target << position_init;
  ...
  if (s <= 1) {
    ...
    orientation_d  =  orientation_d.slerp(2 * s/1000, orientation_d_target);
  }
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceP2P::saturateTorqueRate(
